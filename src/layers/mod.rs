...
pub trait RawLayer: std::fmt::Debug {
    ...
    fn forward(&mut self, input: &Array<f32, Self::Input>, train: bool)
    ...
    fn backward(
        &mut self,
        error: &Array<f32, Self::Output>,
        forward_input: &Array<f32, Self::Input>,
    ) -> Array<f32, Self::Input>;
...
pub mod tests {
    ...
    pub fn test_activation_fn<T>(
        mut activation: T,
        input: Vec<f32>,
        expected_out: Vec<f32>,
        error: Vec<f32>,
        expected_err: Vec<f32>,
    ) where
        T: RawLayer<Input = Dim<IxDynImpl>, Output = Dim<IxDynImpl>>,
    {
        let epsilon = 1e-5;

        let input = Array1::<f32>::from_shape_vec(input.len(), input)
            .unwrap()
            .into_dyn();
        let output = activation.forward(&input, false);

        let target = Array1::<f32>::from_shape_vec(expected_out.len(), expected_out)
            .unwrap()
            .into_dyn();
        ...
        let error = Array1::<f32>::from_shape_vec(error.len(), error)
            .unwrap()
            .into_dyn();
        let error_signal = activation.backward(&error, &input);

        let target_signal = Array1::<f32>::from_shape_vec(expected_err.len(), expected_err)
            .unwrap()
            .into_dyn();
