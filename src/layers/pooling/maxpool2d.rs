...
use crate::{
    helpers::conv_helpers::{crop_4d, pad_4d},
    layers::{ParameterGroup, RawLayer},
};
...
impl RawLayer for MaxPool2D {
    ...
    fn forward(&mut self, input: &Array4<f32>, _train: bool) -> Array4<f32> {
        ...
        let output_width =
            ((width - self.kernel_size.1 + (2 * self.padding.1)) / self.stride.1) + 1;
        let output_height =
            ((height - self.kernel_size.0 + (2 * self.padding.0)) / self.stride.0) + 1;
        ...
        for b in 0..batch_size {
            for in_f in 0..in_features {
                ...
                for (i, window) in windows.into_iter().enumerate() {
                    ...
                    let (max_x, max_y) =
                        (max_idx % self.kernel_size.1, max_idx / self.kernel_size.1);
                    max_indices[[b, in_f, y, x]] =
                        (y * self.stride.0 + max_y, x * self.stride.1 + max_x);
                }
            }
        }
